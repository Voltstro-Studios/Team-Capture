...
namespace Team_Capture.Player.Movement
{
    ...
    internal class PlayerTransformSync : NetworkBehaviour
    {
        ...
        [Tooltip("Once buffer is larger catchupThreshold, accelerate by multiplier % per excess entry.")] [Range(0, 1)]
        public float catchupMultiplier = 0.10f;

        private readonly SortedList<double, PlayerTransformSnapshot> clientBuffer = new();

        private readonly Func<PlayerTransformSnapshot, PlayerTransformSnapshot, double, PlayerTransformSnapshot>
            interpolate
        ...
        private void ApplySnapshot(PlayerTransformSnapshot start, PlayerTransformSnapshot goal,
            PlayerTransformSnapshot interpolated)
        {
            rootTransform.localPosition = interpolatePosition ? interpolated.position : goal.position;

            rootTransform.localRotation =
                Quaternion.Euler(0, interpolateRotation ? interpolated.rotation.y : goal.rotation.y, 0);
            cameraTransform.localRotation =
                Quaternion.Euler(interpolateRotation ? interpolated.rotation.x : goal.rotation.x, 0, 0);
        }

        [ClientRpc(channel = Channels.Unreliable, includeOwner = false)]
        private void RpcServerToClientSync(Vector3 position, Vector2 rotation)
        {
        ...
        private void OnServerToClientSync(Vector3 position, Vector2 rotation)
        {
            ...
            PlayerTransformSnapshot snapshot = new(
                timestamp,
                NetworkTime.localTime,
                position, rotation);
