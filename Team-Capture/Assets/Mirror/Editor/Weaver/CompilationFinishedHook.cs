...
namespace Mirror.Weaver
{
    public static class CompilationFinishedHook
    {
        ...
        public static bool
            UnityLogEnabled =
                true; // controls weather Weaver errors are reported direct to the Unity console (tests enable this)
        ...
        private static void OnCompilationFinished(string assemblyPath, CompilerMessage[] messages)
        {
            ...
            string unityEngineCoreModuleDLL = InternalEditorUtility.GetEngineCoreModuleAssemblyPath();
            ...
            if (Program.Process(unityEngineCoreModuleDLL, mirrorRuntimeDll, null, new[] {assemblyPath},
                dependencyPaths.ToArray(), HandleWarning, HandleError))
            {
                WeaveFailed = false;
