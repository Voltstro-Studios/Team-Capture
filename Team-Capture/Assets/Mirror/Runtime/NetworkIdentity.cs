...
{
    /// <summary>
    ///     The NetworkIdentity identifies objects across the network, between server and clients. Its primary data is a
    ///     NetworkInstanceId which is allocated by the server and then set on clients. This is used in network communications
    ///     to be able to lookup game objects on different machines.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         The NetworkIdentity is used to synchronize information in the object with the network. Only the server should
    ///         create instances of objects which have NetworkIdentity as otherwise they will not be properly connected to the
    ///         system.
    ///     </para>
    ///     <para>
    ///         For complex objects with a hierarchy of subcomponents, the NetworkIdentity must be on the root of the
    ///         hierarchy. It is not supported to have multiple NetworkIdentity components on subcomponents of a hierarchy.
    ///     </para>
    ///     <para>NetworkBehaviour scripts require a NetworkIdentity on the game object to be able to function.</para>
    ///     <para>
    ///         The NetworkIdentity manages the dirty state of the NetworkBehaviours of the object. When it discovers that
    ///         NetworkBehaviours are dirty, it causes an update packet to be created and sent to clients.
    ///     </para>
    ...
    ///     <para>
    ///         * If OnSerialize returns true for a NetworkBehaviour, the dirty mask is reset for that NetworkBehaviour, so
    ///         it will not send again until its value changes.
    ///     </para>
    ...
    ///     <para>
    ///         * If the dirty mask for a NetworkBehaviour is zero, the OnDeserialize functions returns without reading any
    ///         more
    ///     </para>
    ///     <para>
    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
        ...
        private static UpdateVarsMessage varsMessage;
...
#pragma warning restore CS0649
        ...
        /// <summary>
        ///     Unique identifier for this particular object instance, used for tracking objects between networked clients and the
        ///     server.
        ///     <para>
        ///         This is a unique identifier for this particular GameObject instance. Use it to track GameObjects between
        ///         networked clients and the server.
        ///     </para>
        /// </summary>
        ...
        /// <summary>
        ///     The <see cref="NetworkConnection">NetworkConnection</see> associated with this NetworkIdentity. This is only valid
        ///     for player and other owned objects on the client.
        /// </summary>
        public NetworkConnection connectionToServer { get; internal set; }

        /// <summary>
        ///     The <see cref="NetworkConnection">NetworkConnection</see> associated with this NetworkIdentity. This is valid for
        ///     player and other owned objects in the server.
        ...
        public NetworkBehaviour[] NetworkBehaviours =>
            networkBehavioursCache = networkBehavioursCache ?? GetComponents<NetworkBehaviour>();
        ...
        public static NetworkIdentity GetSceneIdenity(ulong id)
        {
            return sceneIds[id];
        }
        ...
        private void Awake()
        {
            ...
            if (Application.isPlaying && sceneId != 0)
            {
                ...
                {
                    Debug.LogError(name + "'s sceneId: " + sceneId.ToString("X") + " is already taken by: " +
                                   existing.name +
                                   ". Don't call Instantiate for NetworkIdentities that were in the scene since the beginning (aka scene objects). Otherwise the client won't know which object to use for a SpawnSceneObject message.");
                    Destroy(gameObject);
                }
                else
                {
        ...
        private void OnStartAuthority()
        {
            if (networkBehavioursCache == null)
            {
                Debug.LogError(
                    "Network object " + name +
                    " not initialized properly. Do you have more than one NetworkIdentity in the same object? Did you forget to spawn this object with NetworkServer?",
                    this);
                return;
            }
        ...
        private bool OnSerializeSafely(NetworkBehaviour comp, NetworkWriter writer, bool initialState)
        {
            ...
            if (LogFilter.Debug)
                Debug.Log("OnSerializeSafely written for object=" + comp.name + " component=" + comp.GetType() +
                          " sceneId=" + sceneId.ToString("X") + "header@" + headerPosition + " content@" +
                          contentPosition + " end@" + endPosition + " contentSize=" + (endPosition - contentPosition));
            ...
            NetworkWriter observersWriter, out int observersWritten)
        {
            ...
            {
                Debug.LogError("Only 64 NetworkBehaviour components are allowed for NetworkIdentity: " + name +
            ...
            foreach (NetworkBehaviour comp in NetworkBehaviours)
                ...
                if (initialState || comp.IsDirty())
                {
                    if (LogFilter.Debug)
                        Debug.Log(
                            "OnSerializeAllSafely: " + name + " -> " + comp.GetType() + " initial=" + initialState);
        ...
        internal ulong GetDirtyMask(bool initialState)
        {
            ...
            for (int i = 0; i < components.Length; ++i)
            {
                ...
                if (initialState || comp.IsDirty()) dirtyComponentsMask |= (ulong) (1L << i);
            }
        ...
        internal ulong GetSyncModeObserversMask()
        {
            ...
            {
                NetworkBehaviour comp = components[i];
                if (comp.syncMode == SyncMode.Observers) mask |= 1UL << i;
            }
        ...
        private void OnDeserializeSafely(NetworkBehaviour comp, NetworkReader reader, bool initialState)
        {
            ...
            {
                if (LogFilter.Debug)
                    Debug.Log("OnDeserializeSafely: " + comp.name + " component=" + comp.GetType() + " sceneId=" +
                              sceneId.ToString("X") + " length=" + contentSize);
                comp.OnDeserialize(reader, initialState);
            }
            catch (Exception e)
            {
                // show a detailed error and let the user know what went wrong
                Debug.LogError("OnDeserialize failed for: object=" + name + " component=" + comp.GetType() +
                               " sceneId=" + sceneId.ToString("X") + " length=" + contentSize +
                               ". Possible Reasons:\n  * Do " + comp.GetType() +
                               "'s OnSerialize and OnDeserialize calls write the same amount of data(" + contentSize +
                               " bytes)? \n  * Was there an exception in " + comp.GetType() +
                               "'s OnSerialize/OnDeserialize code?\n  * Are the server and client the exact same project?\n  * Maybe this OnDeserialize call was meant for another GameObject? The sceneIds can easily get out of sync if the Hierarchy was modified only in the client OR the server. Try rebuilding both.\n\n" +
                               e);
            }
        ...
        internal void OnDeserializeAllSafely(NetworkReader reader, bool initialState)
        {
            ...
            for (int i = 0; i < components.Length; ++i)
            {
                ...
                if ((dirtyComponentsMask & dirtyBit) != 0L) OnDeserializeSafely(components[i], reader, initialState);
            }
        }

        // helper function to handle SyncEvent/Command/Rpc
        private void HandleRemoteCall(int componentIndex, int functionHash, MirrorInvokeType invokeType,
            NetworkReader reader)
        {
            ...
            if (0 <= componentIndex && componentIndex < networkBehavioursCache.Length)
            {
                ...
                if (!invokeComponent.InvokeHandlerDelegate(functionHash, invokeType, reader))
                    Debug.LogError("Found no receiver for incoming " + invokeType + " [" + functionHash + "] on " +
                                   gameObject +
        ...
        internal void SetLocalPlayer()
        {
            ...
            foreach (NetworkBehaviour comp in networkBehavioursCache) comp.OnStartLocalPlayer();
        }
        ...
        internal void ClearObservers()
        {
            if (observers != null)
            {
                foreach (NetworkConnection conn in observers.Values) conn.RemoveFromVisList(this, true);
                observers.Clear();
            }
        }
        ...
        /// <summary>
        ///     This causes the set of players that can see this object to be rebuild. The OnRebuildObservers callback function
        ///     will be invoked on each NetworkBehaviour.
        /// </summary>
        ...
        public void RebuildObservers(bool initialize)
        {
            ...
            if (connectionToClient != null && connectionToClient.isReady) newObservers.Add(connectionToClient);
        ...
        [Obsolete("NetworkConnection parameter is no longer needed and nothing is returned")]
        ...
        ///     <para>
        ///         This applies to objects that had authority set by AssignClientAuthority, or
        ///         <see cref="NetworkServer.Spawn">NetworkServer.Spawn</see> with a NetworkConnection parameter included.
        ///     </para>
        ...
        ///     <para>
        ///         This causes hasAuthority to be set on the client that owns the object, and NetworkBehaviour.OnStartAuthority
        ///         will be called on that client. This object then will be in the NetworkConnection.clientOwnedObjects list for
        ///         the connection.
        ///     </para>
        ///     <para>
        ///         Authority can be removed with RemoveClientAuthority. Only one client can own an object at any time. This does
        ///         not need to be called for player objects, as their authority is setup automatically.
        ///     </para>
        /// </summary>
        ...
        internal void MirrorUpdate()
        {
            if (observers != null && observers.Count > 0)
            {
                ...
                OnSerializeAllSafely(false, ownerWriter, out int ownerWritten, observersWriter,
        ...
        {
            foreach (NetworkBehaviour comp in NetworkBehaviours) comp.ClearAllDirtyBits();
        }
        ...
        private void AssignSceneID()
        {
            ...
            if (sceneId == 0 || duplicate)
            {
                ...
                if (BuildPipeline.isBuildingPlayer)
                    throw new Exception("Scene " + gameObject.scene.path +
        ...
        public void SetSceneIdSceneHashPartInternal()
        {
            ...
            if (LogFilter.Debug)
                Debug.Log(name + " in scene=" + gameObject.scene.name + " scene index hash(" + pathHash.ToString("X") +
                          ") copied into sceneId: " + sceneId.ToString("X"));
        }
