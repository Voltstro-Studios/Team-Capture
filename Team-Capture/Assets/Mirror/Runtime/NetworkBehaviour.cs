...
namespace Mirror
{
    ...
    /// <remarks>
    ///     <para>
    ///         This is a MonoBehaviour class so scripts which need to use the networking feature should inherit this class
    ///         instead of MonoBehaviour. It allows you to invoke networked actions, receive various callbacks, and
    ///         automatically synchronize state from server-to-client.
    ///     </para>
    ///     <para>
    ///         The NetworkBehaviour component requires a NetworkIdentity on the game object. There can be multiple
    ///         NetworkBehaviours on a single game object. For an object with sub-components in a hierarchy, the
    ///         NetworkIdentity must be on the root object, and NetworkBehaviour scripts must also be on the root object.
    ///     </para>
    ///     <para>
    ///         Some of the built-in components of the networking system are derived from NetworkBehaviour, including
    ///         NetworkTransport, NetworkAnimator and NetworkProximityChecker.
    ///     </para>
    /// </remarks>
    [AddComponentMenu("")]
    ...
    public class NetworkBehaviour : MonoBehaviour
    {
        ...
        ///     <para>
        ///         This is only true if the object has been spawned. This is different from NetworkServer.active, which is true
        ///         if the server itself is active rather than this object being active.
        ///     </para>
        /// </summary>
        ...
        ///     <para>
        ///         In multiplayer games, there are multiple instances of the Player object. The client needs to know which one
        ///         is for "themselves" so that only that player processes input and potentially has a camera attached. The
        ///         IsLocalPlayer function will return true only for the player instance that belongs to the player on the local
        ///         machine, so it can be used to filter out input for non-local players.
        ///     </para>
        /// </summary>
        ...
        ///     <para>
        ///         For objects that had their authority set by AssignClientAuthority on the server, this will be true on the
        ///         client that owns the object. NOT on other clients.
        ///     </para>
        /// </summary>
        ...
        ///     <para>
        ///         This is assigned at runtime by the network server and will be unique for all objects for that network
        ///         session.
        ///     </para>
        /// </summary>
        ...
        /// <summary>
        ///     The <see cref="NetworkConnection">NetworkConnection</see> associated with this
        ///     <see cref="NetworkIdentity">NetworkIdentity</see>. This is only valid for player and other owned objects on the
        ///     client.
        /// </summary>
        ...
        /// <summary>
        ///     The <see cref="NetworkConnection">NetworkConnection</see> associated with this
        ///     <see cref="NetworkIdentity">NetworkIdentity</see>. This is valid for player and other owned objects on the server.
        ...
        public NetworkIdentity netIdentity
        {
            get
            {
                if (netIdentityCache == null) netIdentityCache = GetComponent<NetworkIdentity>();
        ...
        public int ComponentIndex
        {
            get
            {
                ...
                Debug.LogError(
                    "Could not find component in GameObject. You should not add/remove components in networked objects dynamically",
                    this);
        ...
        public void ClearAllDirtyBits()
        {
            ...
            //   for: 231b/frame
            for (int i = 0; i < syncObjects.Count; ++i) syncObjects[i].Flush();
        }
        ...
        {
            if (Time.time - lastSyncTime >= syncInterval) return syncVarDirtyBits != 0L || AnySyncObjectDirty();
            return false;
        }

        /// <summary>
        ///     Virtual function to override to send custom serialization data. The corresponding function to send serialization
        ///     data is OnDeserialize().
        /// </summary>
        /// <remarks>
        ///     <para>
        ///         The initialState flag is useful to differentiate between the first time an object is serialized and when
        ///         incremental updates can be sent. The first time an object is sent to a client, it must include a full state
        ///         snapshot, but subsequent updates can save on bandwidth by including only incremental changes. Note that SyncVar
        ///         hook functions are not called when initialState is true, only for incremental updates.
        ///     </para>
        ///     <para>
        ///         If a class has SyncVars, then an implementation of this function and OnDeserialize() are added automatically
        ///         to the class. So a class that has SyncVars cannot also have custom serialization functions.
        ///     </para>
        ///     <para>
        ///         The OnSerialize function should return true to indicate that an update should be sent. If it returns true,
        ///         then the dirty bits for that script are set to zero, if it returns false then the dirty bits are not changed.
        ///         This allows multiple changes to a script to be accumulated over time and sent when the system is ready, instead
        ///         of every frame.
        ///     </para>
        /// </remarks>
        ...
        {
            if (initialState) return SerializeObjectsAll(writer);
            return SerializeObjectsDelta(writer);
        }

        /// <summary>
        ///     Virtual function to override to receive custom serialization data. The corresponding function to send serialization
        ///     data is OnSerialize().
        /// </summary>
        ...
        private ulong DirtyObjectBits()
        {
            ...
            for (int i = 0; i < syncObjects.Count; i++)
            {
                ...
                if (syncObject.IsDirty) dirtyObjects |= 1UL << i;
            }
        ...
        ///     <para>
        ///         This happens after OnStartClient(), as it is triggered by an ownership message from the server. This is an
        ///         appropriate place to activate components or functionality that should only be active for the local player, such
        ///         as cameras and input.
        ///     </para>
        /// </summary>
        ...
        ///     <see cref="NetworkIdentity.hasAuthority">NetworkIdentity.hasAuthority</see>.
        ///     <para>
        ///         This is called after <see cref="OnStartServer">OnStartServer</see> and
        ///         <see cref="OnStartClient">OnStartClient.</see>
        ///     </para>
        ///     <para>
        ///         When <see cref="NetworkIdentity.AssignClientAuthority" /> is called on the server, this will be called on the
        ///         client that owns the object. When an object is spawned with
        ///         <see cref="NetworkServer.Spawn">NetworkServer.Spawn</see> with a NetworkConnection parameter included, this
        ...
        ///     <para>
        ///         When NetworkIdentity.RemoveClientAuthority is called on the server, this will be called on the client that
        ///         owns the object.
        ///     </para>
        /// </summary>
        ...
        /// <returns>
        ///     true when overwriting so that Mirror knows that we wanted to rebuild observers ourselves. otherwise it uses
        ///     built in rebuild.
        /// </returns>
        ...
        ///     <para>
        ///         Objects on a host (with a local client) cannot be disabled or destroyed when they are not visibile to the
        ///         local client. So this function is called to allow custom code to hide these objects. A typical implementation
        ///         will disable renderer components on the object. This is only called on local clients on a host.
        ///     </para>
        /// </summary>
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void SendTargetRPCInternal(NetworkConnection conn, Type invokeClass, string rpcName,
        ...
        protected void SendEventInternal(Type invokeClass, string eventName, NetworkWriter writer, int channelId)
        {
            ...
            SyncEventMessage message = new SyncEventMessage
            {
                ...
                componentIndex = ComponentIndex,
                functionHash =
                    GetMethodHash(invokeClass, eventName), // type+func so Inventory.RpcUse != Equipment.RpcUse
                payload = writer.ToArraySegment() // segment to avoid reader allocations
            };
        ...
        {
            if (cmdHandlerDelegates.TryGetValue(cmdHash, out Invoker invoker)) return invoker.invokeFunction;
            return null;
        }
        ...
        protected bool SyncVarGameObjectEqual(GameObject newGameObject, uint netIdField)
        {
            ...
            if (newGameObject != null)
            {
                ...
                if (identity != null)
                {
                    ...
                    if (newNetId == 0)
                        Debug.LogWarning("SetSyncVarGameObject GameObject " + newGameObject +
            ...
            ref uint netIdField)
        {
            ...
            if (LogFilter.Debug)
                Debug.Log("SetSyncVar GameObject " + GetType().Name + " bit [" + dirtyBit + "] netfieldId:" +
                          netIdField + "->" + newNetId);
        ...
        protected bool SyncVarNetworkIdentityEqual(NetworkIdentity newIdentity, uint netIdField)
        {
            ...
            if (newIdentity != null)
            {
                ...
                if (newNetId == 0)
                    Debug.LogWarning("SetSyncVarNetworkIdentity NetworkIdentity " + newIdentity +
                                     " has a zero netId. Maybe it is not spawned yet?");
            }
            ...
            ulong dirtyBit, ref uint netIdField)
        {
            ...
            if (LogFilter.Debug)
                Debug.Log("SetSyncVarNetworkIdentity NetworkIdentity " + GetType().Name + " bit [" + dirtyBit +
                          "] netIdField:" + netIdField + "->" + newNetId);
            SetDirtyBit(dirtyBit);
            netIdField = newNetId;
