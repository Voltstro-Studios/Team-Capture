...
{
    /// <summary>
    ///     A High level network connection. This is used for connections from client-to-server and for connection from
    ///     server-to-client.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         A NetworkConnection corresponds to a specific connection for a host in the transport layer. It has a
    ///         connectionId that is assigned by the transport layer and passed to the Initialize function.
    ///     </para>
    ///     <para>
    ///         A NetworkClient has one NetworkConnection. A NetworkServerSimple manages multiple NetworkConnections. The
    ///         NetworkServer has multiple "remote" connections and a "local" connection for the local client.
    ///     </para>
    ///     <para>
    ///         The NetworkConnection class provides message sending and handling facilities. For sending data over a
    ///         network, there are methods to send message objects, byte arrays, and NetworkWriter objects. To handle data
    ///         arriving from the network, handler functions can be registered for message Ids, byte arrays can be processed by
    ///         HandleBytes(), and NetworkReader object can be processed by HandleReader().
    ///     </para>
    ///     <para>
    ///         NetworkConnection objects also act as observers for networked objects. When a connection is an observer of a
    ///         networked object with a NetworkIdentity, then the object will be visible to corresponding client for the
    ///         connection, and incremental state changes will be sent to the client.
    ///     </para>
    ///     <para>
    ///         There are many virtual functions on NetworkConnection that allow its behaviour to be customized.
    ///         NetworkClient and NetworkServer can both be made to instantiate custom classes derived from NetworkConnection
    ///         by setting their networkConnectionClass member variable.
    ///     </para>
    /// </remarks>
    public abstract class NetworkConnection
    {
        ...
        /// <remarks>
        ///     <para>
        ///         On a server, this Id is unique for every connection on the server. On a client this Id is local to the
        ///         client, it is not the same as the Id on the server for this connection.
        ///     </para>
        ///     <para>
        ///         Transport layers connections begin at one. So on a client with a single connection to a server, the
        ///         connectionId of that connection will be one. In NetworkServer, the connectionId of the local connection is
        ///         zero.
        ///     </para>
        ///     <para>
        ///         Clients do not know their connectionId on the server, and do not know the connectionId of other clients on
        ///         the server.
        ///     </para>
        /// </remarks>
        ...
        [Obsolete(
            "hostId will be removed because it's not needed ever since we removed LLAPI as default. It's always 0 for regular connections and -1 for local connections. Use connection.GetType() == typeof(NetworkConnection) to check if it's a regular or local connection.")]
        public int hostId = -1;
        ...
        ///     <para>
        ///         This property is read-only. It is set by the system on the client when ClientScene.Ready() is called, and set
        ///         by the system on the server when a ready message is received from a client.
        ///     </para>
        ///     <para>
        ///         A client that is ready is sent spawned objects by the server and updates to the state of spawned objects. A
        ///         client that is not ready is not sent spawned objects.
        ///     </para>
        /// </summary>
        public bool isReady;
        ...
        /// <remarks>
        ///     <para>
        ///         Warning: this can be a lot of data and can be very slow. Both incoming and outgoing messages are logged. The
        ///         format of the logs is:
        ///     </para>
        ///     <para>
        ///         ConnectionSend con:1 bytes:11 msgId:5 FB59D743FD120000000000 ConnectionRecv con:1 bytes:27 msgId:8
        ///         14F21000000000016800AC3FE090C240437846403CDDC0BD3B0000
        ///     </para>
        ///     <para>
        ///         Note that these are application-level network messages, not protocol-level packets. There will typically be
        ///         multiple network messages combined in a single protocol packet.
        ///     </para>
        /// </remarks>
...
#pragma warning restore 618
        ...
        /// <summary>
        ///     Obsolete: Use <see cref="NetworkClient.UnregisterHandler{T}" /> and
        ///     <see cref="NetworkServer.UnregisterHandler{T}" /> instead
        /// </summary>
        [EditorBrowsable(EditorBrowsableState.Never)]
        ...
        /// <summary>
        ///     This sends a network message with a message ID on the connection. This message is sent on channel zero, which by
        ///     default is the reliable channel.
        /// </summary>
        ...
        protected internal static bool ValidatePacketSize(ArraySegment<byte> segment, int channelId)
        {
            ...
            {
                Debug.LogError("NetworkConnection.ValidatePacketSize: cannot send packet larger than " +
                               Transport.activeTransport.GetMaxPacketSize(channelId) + " bytes");
                return false;
            }
        ...
        {
            foreach (NetworkIdentity identity in visList) identity.RemoveObserverInternal(this);
            visList.Clear();
        }
        ...
        ///     <para>
        ///         Network connections used by the NetworkClient and NetworkServer use this function for handling network
        ///         messages.
        ///     </para>
        /// </summary>
        /// <typeparam name="T">The message type to unregister.</typeparam>
        ...
        /// <summary>
        ///     This function allows custom network connection classes to process data from the network before it is passed to the
        ///     application.
        /// </summary>
            ...
            if (MessagePacker.UnpackMessage(reader, out int msgType))
            {
                ...
                if (logNetworkMessages)
                    Debug.Log("ConnectionRecv " + this + " msgType:" + msgType + " content:" +
                              BitConverter.ToString(buffer.Array, buffer.Offset, buffer.Count));

                // try to invoke the handler for that message
                if (InvokeHandler(msgType, reader, channelId)) lastMessageTime = Time.time;
            }
            else
            {
