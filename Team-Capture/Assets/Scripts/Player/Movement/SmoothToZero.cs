...
{
    public class SmoothToZero : MonoBehaviour
    {
        ...
        private void Smooth()
        {
            ...
            localTransform.localPosition = Vector3.MoveTowards(localPosition, Vector3.zero, distance * smoothRate * Time.deltaTime);
            
            distance = Mathf.Max(1f, Quaternion.Angle(transform.localRotation, Quaternion.identity));
            localTransform.localRotation = Quaternion.RotateTowards(localTransform.localRotation, Quaternion.identity, distance * smoothRate * Time.deltaTime);
        }
