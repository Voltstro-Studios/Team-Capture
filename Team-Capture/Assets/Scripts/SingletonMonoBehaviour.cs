...
// ReSharper disable once CommentTypo
			...
			get
			{
				...
				//Finding an existing instance
				{
					...
					if (instances.Length > 1)
					{
						...
						//Tell all but the first instance that they're duplicates
	...
	#region Singleton Implementation
		...
		/// <summary>
		///     Creates a new <see cref="GameObject"/> in the scene and adds a <typeparamref name="T"/> <see cref="Component"/> to it.
		/// </summary>
		[MustUseReturnValue]
		private static T CreateNewInstance()
		{
			GameObject containerGo = new GameObject($"{typeof(T).Name} (Singleton)");
